...
void LivoxMsgCbk1(const livox_ros_driver::CustomMsgConstPtr& livox_msg_in) {
  ...
  for(unsigned int i = 0; i < pcd_size; ++i){
    ...
    *(reinterpret_cast<float*>(ptr +  18))  = (float)(livox_msg_in->points[i].offset_time+livox_msg_in->timebase) / (float)1e9;
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber sub_livox_msg1 = nh.subscribe<livox_ros_driver::CustomMsg>(
      "/scan", 100, LivoxMsgCbk1);
  pub_pcl_out1 = nh.advertise<sensor_msgs::PointCloud2>("/livox/lidar", 100);
